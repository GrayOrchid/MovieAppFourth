...
export default class MoviesAPI {
	...
	static async getTrendPeople(setPeopleArr) {
		let response = await axios.get(`https://api.themoviedb.org/3/person/popular?api_key=${key}&language=ru-RUS `)
		setPeopleArr(response.data.results)
		return response
	}
	...
	static async getMovieDetails(movieId, setGenres, setComapanies, setCountries, setMovie) {
		let response = await axios.get(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${key}&language=ru-RUS`)
